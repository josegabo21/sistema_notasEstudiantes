<section>
    ...
    <form method="post" action="{{ route('profile.update') }}" class="mt-6 space-y-6" enctype="multipart/form-data">
        ...
        <div>
            <x-input-label for="telefono_representante" :value="__('Teléfono')" />
            <x-text-input id="telefono_representante" name="telefono_representante" type="text" class="mt-1 block w-full" :value="old('telefono_representante', $user->telefono_representante)" placeholder="Ingrese el teléfono" pattern="[0-9]{10}" title="Debe ingresar un número de 10 dígitos sin el código de país" maxlength="10" />
            <small class="form-text text-muted">Ejemplo: 1234567890</small>
            <x-input-error class="mt-2" :messages="$errors->get('telefono_representante')" />
        </div>
