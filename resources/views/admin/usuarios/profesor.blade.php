                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                          <form id="delete-form-{{ $profesor->id }}" action="{{ route('admin.usuarios.profesor.destroy', $profesor->id) }}" method="POST" style="display: inline;">
                                              @csrf
                                              @method('DELETE')
                                              <div class="d-flex justify-content-center" style="white-space: nowrap;">
                                              <button type="button" class="btn btn-warning mr-2" data-toggle="modal" data-target="#assignGradeModal" data-id="{{ $profesor->id }}" data-nombre="{{ $profesor->nombre }}" data-apellido="{{ $profesor->apellido }}" data-grado="{{ $profesor->grado_asignado }}">Asignar Grado</button>
                                              <button class="btn btn-danger" type="button" onclick="confirmDelete({{ $profesor->id }})">Eliminar Profesor</button>
                                              </div> 
                                            </form>
                                        </td>                        
                                    </tr>
    ...
    function confirmDelete(profesorId) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                ...
                document.getElementById('delete-form-' + profesorId).submit();
            }
        });
    }
</script>
